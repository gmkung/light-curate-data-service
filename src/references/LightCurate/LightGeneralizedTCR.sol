...
contract LightGeneralizedTCR is IArbitrable, IEvidence {
    ...
    mapping(bytes32 => mapping(uint256 => DisputeData))
        public requestsDisputeData; // Maps an item and a request to the data of the dispute related to them. requestsDisputeData[itemID][requestIndex]
    ...
    function addItem(string calldata _item) external payable {
        ...
        IArbitrator arbitrator = arbitrationParamsChanges[
            arbitrationParamsIndex
        ].arbitrator;
        bytes storage arbitratorExtraData = arbitrationParamsChanges[
            arbitrationParamsIndex
        ].arbitratorExtraData;
        ...
        emit RequestSubmitted(
            itemID,
            getEvidenceGroupID(itemID, item.requestCount - 1)
        );
    ...
    function removeItem(
        ...
        uint256 evidenceGroupID = getEvidenceGroupID(
            _itemID,
            item.requestCount - 1
        );
    ...
    function challengeRequest(
        ...
        DisputeData storage disputeData = requestsDisputeData[_itemID][
            lastRequestIndex
        ];
        require(
            disputeData.status == DisputeStatus.None,
            "The request should not have already been disputed."
        );

        ArbitrationParams storage arbitrationParams = arbitrationParamsChanges[
            request.arbitrationParamsIndex
        ];
        ...
        // Casting is safe here because this line will never be executed in case
        // totalCost > type(uint128).max, since it would be an unpayable value.
        item.sumDeposit = item.sumDeposit.addCap(uint128(totalCost)).subCap(
            uint128(arbitrationCost)
        );
        ...
        arbitratorDisputeIDToItemID[address(arbitrator)][
            disputeData.disputeID
        ] = _itemID;

        uint256 metaEvidenceID = 2 *
            request.arbitrationParamsIndex +
            uint256(request.requestType);
    ...
    function fundAppeal(bytes32 _itemID, Party _side) external payable {
        ...
        uint256 multiplier;
        {
            ...
            Party winner = Party(
                arbitrator.currentRuling(disputeData.disputeID)
            );
    ...
    function withdrawFeesAndRewards(
        ...
        if (_roundID == disputeData.roundCount - 1) {
            ...
            uint256 totalFeesInRound = round.amountPaid[
    ...
    function rule(uint256 _disputeID, uint256 _ruling) external {
        ...
        DisputeData storage disputeData = requestsDisputeData[itemID][
            lastRequestIndex
        ];
        require(
            disputeData.status == DisputeStatus.AwaitingRuling,
            "The request must not be resolved."
        );
        ...
        require(
            address(arbitrationParams.arbitrator) == msg.sender,
        ...
        if (winner == Party.None) {
            ...
            item.status = item.status == Status.RegistrationRequested
                ? Status.Absent
                : Status.Registered;
     ...
     */
    function changeChallengePeriodDuration(
        uint256 _challengePeriodDuration
     ...
     */
    function changeRemovalBaseDeposit(
     ...
     */
    function changeSharedStakeMultiplier(
     ...
     */
    function changeWinnerStakeMultiplier(
     ...
     */
    function changeLoserStakeMultiplier(
     ...
     */
    function changeRelayerContract(
        address _relayerContract
    ) external onlyGovernor {
        relayerContract = _relayerContract;
    }
    ...
    function _doChangeArbitrationParams(
        ...
        emit MetaEvidence(
            2 * arbitrationParamsChanges.length,
            _registrationMetaEvidence
        );
        emit MetaEvidence(
            2 * arbitrationParamsChanges.length + 1,
            _clearingMetaEvidence
        );
    ...
    function contribute(
        ...
        Round storage round = requestsDisputeData[_itemID][_requestID].rounds[
            _roundID
        ];
    ...
    function arbitrator() external view returns (IArbitrator) {
        return
            arbitrationParamsChanges[arbitrationParamsChanges.length - 1]
                .arbitrator;
    }
    ...
    function getItemInfo(
        ...
        external
        view
        returns (Status status, uint256 numberOfRequests, uint256 sumDeposit)
    {
        Item storage item = items[_itemID];
        ...
        )
    {
        ...
        (disputed, disputeID, numberOfRounds, ruling) = getRequestDisputeData(
            _itemID,
            _requestID
        );
        ...
        (
            ...
            metaEvidenceID
        ) = getRequestArbitrationParams(_itemID, _requestID);
     ...
     * @return resolved True if the request was executed and/or any raised disputes were resolved.
     */
    function getRequestResolvedStatus(
        bytes32 _itemID,
        uint256 _requestID
    ) internal view returns (bool resolved) {
        Item storage item = items[_itemID];
        ...
        )
    {
        ...
        hasPaid[uint256(Party.Requester)] =
            appealed ||
            round.sideFunded == Party.Requester;
        hasPaid[uint256(Party.Challenger)] =
            appealed ||
            round.sideFunded == Party.Challenger;
...
contract LightGTCRFactory {
    ...
    ) internal returns (LightGeneralizedTCR instance) {
        assembly {
            let ptr := mload(0x40)
            mstore(
                ptr,
